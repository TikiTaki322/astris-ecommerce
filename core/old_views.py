...
- убрать коммент ProductImage в core/models.py, убрать коммент для рега модели в admin.py, сделать миграции и обкатать логику.
...
1. Race conditions: В OrderItemCreateView есть потенциальная проблема - между проверкой product.quantity и сохранением может пройти время. 
    Лучше использовать F() expressions или select_for_update().
2. Транзакции: Операции создания/обновления заказов стоило бы обернуть в транзакции, особенно в OrderBuilderService.
3. Валидация: В session_order нет валидации данных - можно добавить проверки на корректность quantity, цен и т.д.
4. Duplicate код: Логика работы с quantity повторяется для db/session случаев.
